...
def compute_smoothness_loss(rgb_pred, mask):
...
def compute_sharpness_loss(rgb_pred, rgb_gt, mask):
    rgb_pred = rgb_pred * mask
    rgb_gt = rgb_gt * mask
    ...
    return sharpness_loss * args.finetune.sharp * mask.float().mean()
...
def calc_style_loss(rgb: torch.Tensor, rgb_gt: torch.Tensor, args, loss_dict, neg_texts, opt, background):
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        future_smoothness = executor.submit(compute_smoothness_loss, rgb_pred, mask)
        future_sharpness = executor.submit(compute_sharpness_loss, rgb_pred, rgb_gt, mask)
